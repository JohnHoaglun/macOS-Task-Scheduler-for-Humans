...
* **Identity** — the job name and the managed label. The name is forced to
  ...
  `io.github.macos-task-scheduler.user.<slug>-<8-hex>` (the name slug plus the
   first 8 hex characters of the job's UUID) and shown as single-line plain
   text: type the name and the label updates itself — it is a display, not an
   input field. The name field is sized for comfortable reading (≈30 characters
