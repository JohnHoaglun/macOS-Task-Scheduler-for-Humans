...
2. **Façade (`TaskCommandService`; signatures pinned at stage 0, implemented by 1A):**
...
|---|---|---|---|---|
| 0 | `build` (serial) | `application/external_import.py` DTO; façade signatures + error/acknowledgement semantics; `JobService.import_job` create-only invariants; interface tests. | Import contract pinned; `make check` green. | None (defines the shared surface). |
| 1 | 1A `faster` | Service preview/commit + catalog hardening (`task_command_service.py`, `job_service.py`, `fakes.py`) + service tests. | Every import invariant holds against `FakeTaskWorld`; no source/deployment side effects. | Stage 0. |
| 1 | 1B `faster` | `mactask import` + `--acknowledge-partial` (`cli/app.py`, `cli/render.py`) + CLI tests (exit 0/2, disclosure). | Supported/partial/invalid/duplicate paths meet the exit + output contract. | Stage 0. |
| 1 | 1C `faster` | `import_controller.py`, `import_preview_dialog.py`, `main_window.py` action/wiring + GUI tests. | Action external-row-only; acknowledgement gate + cancellation verified; no side effects. | Stage 0. |
| 1 | 1D `faster` | README, `docs/architecture.md`, `docs/development.md`. | Docs disclose catalog-only, source-preservation, acknowledgement, test boundaries. | Stage 0. |
...
|---|---|---|
| 1A ↔ 1B, 1C | `ExternalPlistImportPreview` + façade methods | Immutable preview; `requires_acknowledgement` drives the gate; commit regenerates the durable UUID. Owner: Stage 0. |
| 1B ↔ 1C | Warning/unsupported disclosure | Both render every warning and unsupported key; `--acknowledge-partial` (CLI) and the checkbox (GUI) are the two equivalent acknowledgement mechanisms. Owner: Stage 0. |
| 1A ↔ 1D | User-visible guarantees | Catalog-only, source-untouched, no deployment; warnings fully disclosed. Owner: Stage 0. |
| 1A ↔ existing `JobService` | `import_job` semantics | Create-only: no existing UUID path AND no other record owning the label. Owner: 1A (contract pinned at Stage 0). |
| 1C ↔ `MainWindow` | Action availability | Enabled only for discovered + unmanaged + representable external rows. Owner: 1C. |
