...
8. **Compatibility.** `DirectTestResult` gains `report: DiagnosticReport` (empty-report default factory); `diagnostics` stays the legacy flat list, so CLI rendering and existing consumers are unchanged. `diagnostic_service.py` re-exports `Diagnostic`/`DiagnosticSeverity` (and the new models) so every existing import path keeps working.
