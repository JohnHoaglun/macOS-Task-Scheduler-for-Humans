...
6. **Filter semantics — AND groups, OR values.** Search (case-insensitive substring over name, label, and shell-quoted command) and each active filter group must match; selecting multiple values within one group broadens that group (OR within a group, AND across groups/search).
