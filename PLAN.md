...
### Increment 23 — Remaining Python Ecosystem Detectors (§58) — IN PROGRESS (v0.0.23)
...
- **Default registry order (pinned):** `core → uv → poetry → pipenv → pyenv → conda → homebrew`. `default_python_detectors()` is the only place that assembles the registry; it imports the four new detector modules **lazily inside the function** (avoids a top-level import cycle, since each new detector module imports base types from `python_detection`).
...
**Shared-surface inventory (pinned; each owned by exactly one lane):**
- `python_detection.py` — Stage 0 (`build`) owns `DetectorKind`, `PythonDetectionRoots`, `DetectionContext`, `nearest_marker_root`, `detect_python`; Wave 2 (`build`) owns the registry assembly in `default_python_detectors()`. No Wave 1 lane edits this file.
- `platform/macos/__init__.py` — Stage 0 (`build`) adds the `PythonDetectionRoots` export only.
- New detector modules — Wave 1, one per lane, no cross-lane edits: `pyenv_python_detector.py`, `conda_python_detector.py`, `pipenv_python_detector.py`, `homebrew_python_detector.py`. Each imports base types + `nearest_marker_root` + `_is_usable` from `python_detection` and defines exactly one detector class with its pinned `kind`.
...
- Shared test helper — Stage 0 (`build`): `tests/unit/platform/python_detection_fakes.py` (non-collected) holds `FakePythonDetectorFilesystem` (moved out of `test_python_detection.py`) plus `make_executable`/`lookup`/`detect` helpers; every new per-detector test file imports it (no duplication).
- Per-detector test files — Wave 1, one per lane: `test_pyenv_python_detector.py`, `test_conda_python_detector.py`, `test_pipenv_python_detector.py`, `test_homebrew_python_detector.py` — isolated `.detect()` tests against a `DetectionContext` with a `tmp_path`-based `PythonDetectionRoots`.
- Registry/composition tests — Wave 2 (`build`): extend `test_python_detection.py` with a registry-order + cross-detector dedupe/provenance/notes class.

**Lane map (wave order decided at planning):**
- **Stage 0 (serial, `build`):** the contract above; `default_python_detectors()` still returns `(core, uv, poetry)` (unchanged) so existing `detect_python` behavior and all existing tests are byte-identical. Stop: `make check` green, existing `test_python_detection.py` assertions unchanged, 100% coverage of the new contract lines.
- **Wave 1 (parallel, `faster`):** four detector modules + four test files (each self-contained against the Stage 0 contract) plus a docs lane (README Python section, `docs/architecture.md` registry + `PythonDetectionRoots`, `docs/development.md` detector test conventions). Stop: each detector's isolated tests pass, 100% coverage of the new module, no edits to `python_detection.py`/`__init__.py`/the registry.
- **Wave 2 (serial, `build`):** compose `default_python_detectors()` to `(core, uv, poetry, pipenv, pyenv, conda, homebrew)` via lazy imports; update the `detect_python`/`default_python_detectors` docstrings; add registry-order + cross-detector composition tests (ordering, dedupe by spelling, multi-detector provenance, note order, source preservation, `project_environment_candidate` anchor). Stop: `make check` green, 100% coverage.
- **Wave 3 (serial, `build`):** verification + closeout — `make check` (ruff/mypy strict/pytest), explicit 100% package coverage, source-size review (all new modules ≪ 500 lines), **test/code ratio ≤ 75% (hard cap — see gate below)**, docs finalized, version `0.0.22 → 0.0.23` registry + stale-reference grep, commit, push.
